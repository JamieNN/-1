...
function handleExplodeAction(row,col,game,cellEl,tableEl){
    ...
    panelEl.innerHTML=` <h3>你失败了，用时${game.seconds}秒</h3>`;
...
function handleWin(row,col,game,cellEl,tableEl){
    ...
    gameEl.append(panelEl);
    panelEl.innerHTML=` <h3>恭喜你，你在${game.seconds}秒完成比赛</h3>`;
...
function renderWelcome(){
    ...
    <button id='leve20'>中级</button>
    <button id='advance'>高級</button>
    </div>
    `;
    let buttonEl=gameEl.querySelector('button#level0');
    buttonEl.onclick=()=>{    
        let game = new Game(8, 6, 0.05);
    game.shuffle();
    renderTable(game);}

    buttonEl=gameEl.querySelector('button#leve20');
    buttonEl.onclick=()=>{    
        let game = new Game(12, 20, 0.1);
    game.shuffle();
    renderTable(game);}
